...
def get_base_url(request):
    ...
    En producción (DEBUG=False) usa el dominio del request.
    En desarrollo (DEBUG=True) usa la IP local detectada o la configuración DEV_SERVER_URL.
    """
    if settings.DEBUG:
        # Si hay una variable de entorno o setting con la URL de desarrollo, la usamos
        dev_url = getattr(settings, 'DEV_SERVER_URL', None)
        if dev_url:
            return dev_url.rstrip('/')
        # Si no, intentamos obtener la IP local
        try:
        ...
        # En producción, usamos el dominio real (https)
...
# ============================================================================
# VISTAS PRINCIPALES (protegidas con login)
# ============================================================================
...
def panel_apps_moviles(request):
    ...
    stats = {
        ...
        'empresas_sin_app': empresas.count() - empresas.filter(app_generada=True).count(),
...
def generar_app_empresa(request, empresa_slug):
    ...
    if request.method == 'POST':
        try:
            ...
            # Generar configuración JSON base
            ...
            apps_qr_dir = static_dir / 'apps_qr'
            for d in [static_dir, apps_config_dir, apps_qr_dir]:
...
# ============================================================================
# FUNCIONES DE GENERACIÓN DE QR (con URL dinámica)
# ============================================================================

def _crear_qr_unico(request, empresa, total_clientes):
    """Crea un QR con token único usando la URL base dinámica."""
    ...
    print(f"🔗 QR generado con URL: {url_publica}")
...
def _ver_qr_universal(request, empresa, total_clientes, total_sectores):
    """Muestra el QR generado con la URL base dinámica."""
    base_url = get_base_url(request)
    token_unico = hashlib.sha256(
        f"{empresa.slug}-UNIVERSAL-{time.time()}".encode()
    ).hexdigest()[:32]
    ...
    qr_info = {
        ...
        'v': '1',
        'servidor_url': base_url,  # Clave para la app
    }
    
    qr_data = json.dumps(qr_info, separators=(',', ':'), ensure_ascii=False)
    
    # Obtener configuración para mostrar en el template
    try:
...
# ============================================================================
# APIS PARA LA APP MÓVIL (públicas, usan token de sesión)
# ============================================================================
...
descargar_config_grande = api_descargar_config
api_publica_config = api_descargar_config
...
# ============================================================================
# VISTAS DE GESTIÓN DE DISPOSITIVOS
# ============================================================================
...
def debug_config_json(request, empresa_slug):
    ...
    config_json = empresa.generar_config_app()
    debug_info = {
        'empresa': {'nombre': empresa.nombre, 'slug': empresa.slug, 'id': empresa.id},
...
# ============================================================================
# VISTA DE LOGIN PARA DISPOSITIVOS (endpoint público)
# ============================================================================
